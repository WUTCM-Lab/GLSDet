...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # pred[:, 2:]表示宽高,tl表示交集方块的左下角，br表示交集方块的右上角
        br = torch.min(
            (pred[:, :2] + pred[:, 2:] / 2), (target[:, :2] + target[:, 2:] / 2)
        )
        ...
        # en可以表示是否有交集，若不存在交集，那么tl>br，那么就会是0
...
class YOLOLoss(nn.Module):    
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        if (anchor_matching_gt > 1).sum() > 0:
            ...
            cost_argmin_index = matching_matrix[cost_argmin, :].sum(1) == 0
            cost_argmin = cost_argmin[cost_argmin_index]
            # if cost_argmin.sum() > 0 :
            matching_matrix_ = matching_matrix[cost_argmin,:]
            matching_matrix_[:, anchor_matching_gt > 1] = 1.0
            matching_matrix[cost_argmin, :] = matching_matrix_
            del matching_matrix_
            # matching_matrix[cost_argmin, anchor_matching_gt > 1] = 1.0

        if (matching_matrix.sum(1) < 1).sum() > 0:
            # matching_matrix_miss = matching_matrix.sum(1) < 1
            # cost_miss = cost[matching_matrix_miss, :]
            cost_miss = cost[matching_matrix.sum(1) < 1, :]
            cost_miss[:, matching_matrix.sum(0) < 1] = cost_miss[:, matching_matrix.sum(0) < 1] - 50000
            _, cost_miss_argmin = torch.min(cost_miss[:, :], dim=1)
            matching_matrix[matching_matrix.sum(1) < 1, cost_miss_argmin] = 1.0
        #------------------------------------------------------------#
