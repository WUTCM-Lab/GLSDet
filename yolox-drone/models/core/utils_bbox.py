...
def decode_outputs_cls_sigmoid(outputs, input_shape):
    grids = []
    strides = []
    hw = [x.shape[-2:] for x in outputs]
    # ---------------------------------------------------#
...
criterion = [0, 32.0 ** 2, 64 ** 2]
criterion_eval = torch.ones(OBJECT_RATIO.size())
criterion_small_index = torch.where(OBJECT_SIZE < criterion[1])
criterion_large_index = torch.where(OBJECT_SIZE > criterion[2])
...
def non_max_suppression(prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5,
    ...
    for i, image_pred in enumerate(prediction):
        ...
        #     if criterion_eval[int(c)] == 0:
        #         _, index = soft_nms(detections_class[:, :4],
        #                        detections_class[:, 4] * detections_class[:, 5], 0.65)
        #     else:
        #         _, index = nms(detections_class[:, :4],
        #                                detections_class[:, 4] * detections_class[:, 5], 0.65)
        #     detections_temp = detections_class[index]
