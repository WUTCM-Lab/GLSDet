...
from models.ffa.darknet import BaseConv, CSPDarknet, CSPLayer, DWConv
from models.ffa.ffa import FFA


class YOLOXHead(nn.Module):
    def __init__(self, num_classes, width=1.0, in_channels=[256, 512, 1024, 256], act="silu", depthwise=False, ):
        super().__init__()
        ...
        self.ftt = FTT(int(width * in_channels[0]))
    ...
    def forward(self, inputs):
        ...
        # ---------------------------------------------------#
        processed_inputs = []
        zz = self.ftt(inputs[1], inputs[2])
        for k, x in enumerate(inputs):
            print(x.shape)
            if k == 0:
                processed_inputs.append(self.csp(x))
            else:
                processed_inputs.append(self.stems[k-1](x))
        processed_inputs[0] = processed_inputs[0] + self.upsample(zz)
