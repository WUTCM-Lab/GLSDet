...
class TOODHead(nn.Module):
    def __init__(self, num_class, width=1.0, in_channels=[256, 512, 1024], stacked_convs=6, act='silu', depthwise=True):
        super().__init__()
        ...
        self.tood_cls = nn.Conv2d(self.feat_channels, self.num_class, 3, padding=1)
    ...
    def forward(self, feats):
        ...
        for idx, x in enumerate(feats):
            ...
            obj_output = self.obj_conv[idx](reg_feat)
            print(feat.shape)
            reg_offset = self.reg_offset_module(feat)
            reg_process_feat = self.deform_sampling(reg_feat, reg_offset)
            reg_output = self.tood_reg(reg_process_feat)

            output = torch.cat([reg_output, obj_output, cls_score], 1)
...
class YoloBody(nn.Module):
    def __init__(self, num_classes, phi):
        ...
        self.head = TOODHead(num_classes, width, depthwise=depthwise)
