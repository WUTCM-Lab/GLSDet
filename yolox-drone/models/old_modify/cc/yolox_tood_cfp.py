...
from models.cfp.evc_blocks import RefineBlock
...
class TOODHead(nn.Module):
    def __init__(self, num_classes, width=1.0, in_channels=[256, 512, 1024], stacked_convs=1, act='silu', depthwise=True):
        super().__init__()
        ...
        self.cls_decomp = RefineBlock(self.feat_channels * self.stacked_convs, self.feat_channels)

        self.reg_decomp = RefineBlock(self.feat_channels * self.stacked_convs, self.feat_channels)
...
class YOLOXHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for k, x in enumerate(inputs):
            ...
            #   reg_pred 20, 20, 4
            # ---------------------------------------------------#
            reg_output = self.reg_preds[k](reg_feat + reg_feat_tood)
            # ---------------------------------------------------#
            #   判断特征点是否有对应的物体
            #   obj_pred 80, 80, 1
            #   obj_pred 40, 40, 1
