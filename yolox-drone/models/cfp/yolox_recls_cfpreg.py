...
from models.cfp.reg_evc_blocks import EVCBlock
...
class YOLOXHead(nn.Module):
    def __init__(self, num_classes, width=1.0, in_channels=[256, 512, 1024], act="silu", depthwise=False, ):
        ...
        for i in range(len(in_channels)):
            ...
            self.evc_blocks.append(EVCBlock(int(256 * width), int(256 * width)))
    ...
    def forward(self, inputs):
        ...
        for k, x in enumerate(inputs):
            if k == 0:
                ...
                inputs_processed.append(self.stems[k-1](x))
        ...
        for k, x in enumerate(inputs_processed):
            ...
            # ---------------------------------------------------#
            if k == 0:
                cls_feat = torch.cat([x, self.up_convs[k](feat0_processed)], 1)
            else:
                cls_feat = torch.cat([x, self.up_convs[k](inputs_processed[k-1])], 1)
