...
class YOLOLoss(nn.Module):
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        base_area_weight = 0.1
        targets_areas = torch.prod(reg_targets[:, 2:], 1)
        tmp = 1.0 / (targets_areas + 0.0001)
        area_weights = tmp / tmp.sum()
        final_area_weights = (area_weights * reg_targets.shape[0]
                              * (1 - base_area_weight) + base_area_weight).unsqueeze(-1)

        iou_loss = self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)
        loss_iou = iou_loss.sum()

        objs_loss = self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)
        loss_obj = objs_loss.sum()

        cls_loss = self.bcewithlog_loss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)
        loss_cls = (cls_loss * final_area_weights).sum()
