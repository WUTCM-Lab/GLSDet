...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, strides=[8, 16, 32], fpn_size=[160, 80, 40]):
        super().__init__()
        self.num_classes = num_classes
        self.strides = strides
        self.fpn_size = fpn_size
        self.cls_loss = ClsLoss(num_classes, strides, fpn_size, alpha=0.8)
        self.reg_loss = RegLoss(num_classes, strides, fpn_size, alpha=0.75)

    def forward(self, inputs, labels):
        loss = 1.0 * self.cls_loss(inputs, labels) + 1.0 * self.reg_loss(inputs, labels)
        return loss


class ClsLoss(nn.Module):
    def __init__(self, num_classes, strides=[8, 16, 32], fpn_size=[160, 80, 40], alpha=0.5):
        super().__init__()
        self.num_classes = num_classes
        self.strides = strides
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for id_b in range(len(fg_masks)):
            ...
            for k, size in enumerate(self.fpn_size):
                ...
                weight = level_weights[id_b][k]
                loss += weight * loss_cls
                index += size * size
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        # -------------------------------------------------------#
        cls_preds_ = cls_preds_.float().unsqueeze(0).repeat(num_gt, 1, 1) * obj_preds_.unsqueeze(0).repeat(
            num_gt, 1, 1)
        ...
        cost = 3.0 * pair_wise_cls_loss + 1 * pair_wise_ious_loss + 100000.0 * (~is_in_boxes_and_center).float()
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for id_b in range(len(fg_masks)):
            ...
            for k, size in enumerate(self.fpn_size):
                ...
                loss_obj = (self.bce_loss(obj_preds[id_b, index:index+size*size, ...],
                                                 obj_targets[id_b][index:index+size*size])).sum()
                # loss_cls = (self.bcewithlog_loss(cls_preds[id_b, index:index+size*size, ...][fg_mask],
                #                                  cls_targets[id_b][index_target:index_target+samples_num[id_b][k]])).sum()

                weight = level_weights[id_b][k]
                loss += weight * (reg_weight * loss_iou + loss_obj)
                index += size * size
                index_target += samples_num[id_b][k]
        num_fg = max(num_fg, 1)
        return loss / num_fg
