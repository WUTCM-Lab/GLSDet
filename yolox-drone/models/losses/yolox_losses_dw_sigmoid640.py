...
def py_focal_loss_with_logits(pred,
                            target=None,
                            gamma=2.0,
                            alpha=0.25,
                            one_hot_target=None):
    """PyTorch version of `Focal Loss <https://arxiv.org/abs/1708.02002>`_.
    Different from `py_sigmoid_focal_loss`, this function accepts logist
    as input.
    ...
    ops = nn.BCEWithLogitsLoss(reduction="none")
    loss = ops(pred, target) * focal_weight
    return loss
...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, strides=[8, 16, 32], fpn_size=[80,40,20]):
        super().__init__()
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                is_in_box = outputs.new_zeros(total_num_anchors).bool()
            else:
                # -----------------------------------------------#
                ...
                gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg_img, is_in_box = self.get_assignments(
        ...
        level_weights, samples_num = self.generate_weights(fg_masks)
        print(level_weights)

        loss = 0.0
        for id_b in range(len(fg_masks)):
            index = 0
            index_target = 0
            reg_weight = 5.0
            for k, size in enumerate(self.fpn_size):
                fg_mask = fg_masks[id_b][index:index+size*size]
                is_in_box_mask = is_in_boxes[id_b][index:index+size*size]
                # print(fg_mask, is_in_box_mask)
                outside_box_mask = ~fg_mask & ~is_in_box_mask
                inside_neg_mask = ~fg_mask & is_in_box_mask
                # print(bbox_preds[id_b, index:index+size*size, ...][fg_mask].shape)
                # print(reg_targets[id_b][index_target:index_target+samples_num[id_b][k]].shape)
                loss_iou = (self.iou_loss(bbox_preds[id_b, index:index+size*size, ...][fg_mask],
                                          reg_targets[id_b][index_target:index_target+samples_num[id_b][k]])).sum()
                loss_obj = (self.bce_loss(obj_preds[id_b, index:index+size*size, ...],
                                                 obj_targets[id_b][index:index+size*size])).sum()
                loss_cls = (self.bce_loss(cls_preds[id_b, index:index+size*size, ...][fg_mask],
                                                 cls_targets[id_b][index_target:index_target+samples_num[id_b][k]])).sum()

                loss_cls_neg_outside = (py_focal_loss_with_logits(pred=cls_preds[id_b, index:index + size * size, ...][outside_box_mask],
                                         one_hot_target=torch.zeros_like(cls_preds[id_b, index:index + size * size, ...][outside_box_mask]))).sum()
                loss_cls_neg_inside = (self.bce_loss(cls_preds[id_b, index:index + size * size, ...][inside_neg_mask],
                                                  cls_preds[id_b, index:index + size * size, ...][inside_neg_mask])).sum()
                weight = 1.0
                loss += weight * (reg_weight * loss_iou + loss_obj + loss_cls
                                  + loss_cls_neg_outside + loss_cls_neg_inside)
                index += size * size
                index_target += samples_num[id_b][k]
        num_fg = max(num_fg, 1)
        return loss / num_fg

    @torch.no_grad()
    def generate_weights(self, fg_masks):
        level_weights = []
        all_samples_num = []
        for fg_mask in fg_masks:
            index = 0
            positive_nums = []
            for size in self.fpn_size:
                tmp_data = fg_mask[index:index + size * size]
                index += size * size
                samples_num = tmp_data.sum(dim=0)
                positive_nums.append(samples_num)
            all_samples_num.append(positive_nums)
            positive_nums = torch.tensor(positive_nums)
            max_num = positive_nums.max()
            min_num = positive_nums.min()

            # print(positive_nums)
            # print("max_num: %d, min_num: %d" % (int(max_num), int(min_num)))

            weights = []
            for i in range(int(positive_nums.shape[0])):
                if max_num - min_num == 0:
                    weight = 2.0
                else:
                    weight = 2.0 - (positive_nums[i] - min_num) * 1.0 / (max_num - min_num)
                weights.append(weight)
            level_weights.append(weights)
        return level_weights, all_samples_num
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        return gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg, is_in_box
