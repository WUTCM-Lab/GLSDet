...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, strides=[8, 16, 32]):
        ...
        for i in range(len(strides)):
            self.losses.append(OneYOLOLoss(num_classes, strides=[strides[i]]))
    ...
    def generate_weights(self, fg_masks):
        ...
        for fg_mask in fg_masks:
            positive_nums = fg_mask.sum(dim=0)
            all_samples_num.append(positive_nums)

        temp = torch.tensor(all_samples_num)
        max_num = temp.max()
        min_num = temp.min()

        for i in range(len(all_samples_num)):
            if max_num - min_num == 0:
                weight = 2.0
            else:
                weight = 2.0 - (all_samples_num[i] - min_num) * 1.0 /(max_num - min_num)
            level_weights.append(weight)

        level_weights = torch.tensor(level_weights)
    ...
    def forward(self, inputs, labels=None):
        ...
        for k, input in enumerate(inputs):
            loss, fg_mask = self.losses[k]([input], labels)
            losses.append(loss)
            fg_masks.append(fg_mask)

        level_weights, all_samples_num = self.generate_weights(fg_masks)
        # print(level_weights)
        # print(all_samples_num)
        weights_all = level_weights.sum(dim=0)

        final_loss = 0.0
        for k, loss in enumerate(losses):
            final_loss += level_weights[k] * losses[k] / weights_all

        # print(final_loss)
        return final_loss


class OneYOLOLoss(nn.Module):
    def __init__(self, num_classes, strides=[8, 16, 32]):
        super().__init__()
