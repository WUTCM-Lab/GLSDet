...
class_book, class_num = get_classes('/home/server2/shk/yolox_drone/model_data/uav3.txt')

def get_annotations(txt_path, prob=False, thred = 0.1):
    bbox = []
    ...
    if len(newbbox) == 0:
        curthred = thred
        return get_annotations(txt_path, True, curthred-0.1)

    return np.array(newbbox)
...
def main():
    ...
    dataset_root = '/home/server2/shk/yolox_drone/UAVVal/VOC2007/JPEGImages'
    dataset_anno = '/home/server2/shk/UAVDT/annotations/UAV-benchmark-M-Val.json'
    ufp_images_output_dir = '/home/server2/shk/yolox_drone/0224uavufp/VOC2007/JPEGImages'
    ufp_anno_output_path = '/home/server2/shk/yolox_drone/model_data/0224valufp.json'
    txt_dir = '/home/server2/shk/yolox_drone/new_results/uav_ftt2_30/detection-results'
    ...
    for key in range(size):
        ...
        tmp_txt_path = os.path.join(txt_dir, img_name.replace(".jpg", ".txt").replace("/", "_"))
        print(tmp_txt_path)
        first_results = get_annotations(tmp_txt_path, prob=True)
        result = np.concatenate(first_results)

        # print(float(result[0][0]))
        # print(float(result[0][1]))
        # print(float(result[0][2]))
        # print(float(result[0][3]))
        # print(float(result[0][4]))
        rec, w, h = UnifiedForegroundPacking(result[:, :4], 2, input_shape=[width, height])
        ...
        ufp_img = draw_ufp_result(rec, os.path.join(dataset_root, img_name.replace("/", "_")), img_name, w, h, txt_path)
        cv2.imwrite(os.path.join(ufp_images_output_dir, img_name.replace("/", "_")), ufp_img)
        for bbox in rec:
