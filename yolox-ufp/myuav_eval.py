...
class_book, class_num = get_classes('yolox_drone/model_data/uav3.txt')

### 超参数设置框的置信度阈值
# def get_annotationsFirst(txt_path, prob=False):
#     bbox = []
...
def get_annotationsFirst(txt_path, prob=False, thred = 0.1):
    bbox = []
    for i in range(class_num):
        bbox.append([])
    with open(txt_path, 'r', encoding='utf-8') as f:
    ...
    if len(newbbox) == 0:
        curthred = thred
        return get_annotationsFirst(txt_path, True, curthred-0.1)
...
def main():
    ...
    dataset_root = 'UAVVal/VOC2007/JPEGImages'
    dataset_anno = 'UAV-benchmark-M-Val.json'

    first_txt_dir = 'detection-results0'
    txt_dir = 'detection-results'
    out_dir = 'ufp_results'
   ...
   #  coarse_detecor = init_detector(coarse_detecor_config, coarse_detecor_config_ckpt, device=device)
    ...
    for key in range(size):
        ...
        tmp_txt_path = os.path.join(first_txt_dir, img_name.replace(".jpg", ".txt").replace("/", "_"))
        first_results = get_annotationsFirst(tmp_txt_path, prob=True)
        result = np.concatenate(first_results)

        flag_time1 = time.time()
        rec, w, h = UnifiedForegroundPacking(result[:, :4], 2, input_shape=[width, height])
        all_time += time.time() - flag_time1
        ...
        print(tmp_txt_path)
        second_results = get_annotations(tmp_txt_path, prob=True)

        # print(second_results)

        flag_time2 = time.time()
        finale_results = []
        ...
        # write txt results
        out_txt_path = os.path.join(out_dir, img_name.replace(".jpg", ".txt").replace("/", "_"))
        f_txt_out = open(out_txt_path, 'w')
        ...
        for idx, result in enumerate(finale_results):
            ...
            for bbox in result[keep]:
                ...
                f_txt_out.write("%s %s %s %s %s %s\n" % (CLASSES[idx], float(score), x1, y1, x2, y2))
        f_txt_out.close()
